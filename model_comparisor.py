...
if __name__ == "__main__":
    ...
    arrival_rates = [i / 5 for i in range(1, 51)]
    ...
    v = open("results/v_results.txt", "w")
    w = open("results/w_results.txt", "w")
    wm = open("results/webster_results.txt", "w")
    with open("results/fixed_results.txt", "w") as f:
        for a_ns in arrival_rates:
            for a_ew in arrival_rates:
                ...
                print(f'{a_ns} {a_ew}', file=v, end=' ')
                print(f'{a_ns} {a_ew}', file=w, end=' ')
                print(f'{a_ns} {a_ew}', file=wm, end=' ')
                print(f'{a_ns} {a_ew}', file=f, end=' ')
                
                print(f"Vehicle {vmodel_g_ns} {vmodel_g_ew} {vmodel["cars_accepted_ns"]} {vmodel["cars_accepted_ew"]} {vmodel["average_wait_time_ns"]} {vmodel["average_wait_time_ew"]}", file=v)
                print(f"Waiting {wmodel_g_ns} {wmodel_g_ew} {wmodel["cars_accepted_ns"]} {wmodel["cars_accepted_ew"]} {wmodel["average_wait_time_ns"]} {wmodel["average_wait_time_ew"]}", file=w)
                print(f"Webster {webster_g_ns} {webster_g_ew} {webstermodel["cars_accepted_ns"]} {webstermodel["cars_accepted_ew"]} {webstermodel["average_wait_time_ns"]} {webstermodel["average_wait_time_ew"]}", file=wm)
